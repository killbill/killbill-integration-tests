...
$LOAD_PATH.unshift File.expand_path('mixin-checkers', __dir__)
...
module KillBillIntegrationTests
  class Base < Test::Unit::TestCase
    ...
    DEFAULT_MULTI_TENANT_INFO = { use_multi_tenant: true,
                                  create_multi_tenant: true }.freeze

    def setup_base(user = method_name, tenant_info = DEFAULT_MULTI_TENANT_INFO, init_clock = DEFAULT_KB_INIT_CLOCK, killbill_address = DEFAULT_KB_ADDRESS, killbill_port = DEFAULT_KB_PORT)
      ...
      @proc_account_invoices_nb = proc do |account|
        account.invoices(@options).size
      end
      @proc_invoice_items_nb = proc { |invoice_id| get_invoice_by_id(invoice_id, @options).items.size }
    end
