...
module KillBillIntegrationTests
  class TestInvoice < Base
    ...
    def test_dry_run_change_bp_with_multiple_policies
      ...
      bp.change_plan({ productName: 'Sports', billingPeriod: 'MONTHLY', priceList: 'DEFAULT' }, @user, nil, nil, requested_date, billing_policy, nil, false, @options)
    ...
    def test_create_a_migration_invoice
      ...
      assert(KillBillClient::Model::Invoice.create_migration_invoice(@account.account_id, invoices, '2018-03-15', @user, nil, nil, @options).response.is_a?(Net::HTTPSuccess))
