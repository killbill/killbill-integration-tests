...
module KillBillIntegrationTests
  module AccountHelper
    def get_account(account_id, with_balance, with_balance_and_cba, options)
    ...
    def create_account_with_data(user, data, options)
      ...
      account.external_key = data[:external_key].nil? ? (Time.now.to_i.to_s + '-' + rand(1_000_000).to_s) : data[:external_key]
      ...
      account.is_payment_delegated_to_parent = data[:is_payment_delegated_to_parent] unless data[:is_payment_delegated_to_parent].nil?
      ...
      account.postal_code = data[:postal_code].nil? ? 94_105 : data[:postal_code]
